...
export async function getCards(filters: Partial<CardFilters> = {}): Promise<{ items: CardWithImages[]; total: number; page: number; perPage: number }> {
  ...
  return { items: (data ?? []).map((row) => mapCardRow(asRecord(row))), total: count ?? 0, page, perPage };
}
...
export async function getInquiriesForUser(userId: string) { if (!env.isSupabaseConfigured) return (await readStore()).inquiries.filter((item) => item.userId === userId); const { data, error } = await createAdminClient().from("inquiries").select().eq("user_id", userId).order("created_at", { ascending: false }); if (error) throw error; return (data ?? []).map((item) => snakeToInquiry(asRecord(item))); }
export async function getAllInquiries(filters: InquiryFilters = {}) { const page = filters.page ?? 1; const perPage = filters.perPage ?? 50; if (!env.isSupabaseConfigured) { const inquiries = (await readStore()).inquiries.filter((item) => !filters.status || item.status === filters.status); return { items: inquiries.slice((page - 1) * perPage, page * perPage), total: inquiries.length, page, perPage }; } let query = createAdminClient().from("inquiries").select("*", { count: "exact" }); if (filters.status) query = query.eq("status", filters.status); const { data, error, count } = await query.order("created_at", { ascending: false }).range((page - 1) * perPage, page * perPage - 1); if (error) throw error; return { items: (data ?? []).map((item) => snakeToInquiry(asRecord(item))), total: count ?? 0, page, perPage }; }
...
export async function getInquiryNotes(inquiryId: string, includeInternal = false) { if (!env.isSupabaseConfigured) return (await readStore()).inquiryNotes.filter((note) => note.inquiryId === inquiryId && (includeInternal || !note.isInternal)); let query = createAdminClient().from("inquiry_notes").select().eq("inquiry_id", inquiryId); if (!includeInternal) query = query.eq("is_internal", false); const { data, error } = await query.order("created_at"); if (error) throw error; return (data ?? []).map((note) => snakeToInquiryNote(asRecord(note))); }
...
export async function listUsers() { if (!env.isSupabaseConfigured) return (await readStore()).profiles; const { data, error } = await createAdminClient().from("profiles").select().order("created_at"); if (error) throw error; return (data ?? []).map((item) => snakeToProfile(asRecord(item))); }
...
export async function getAdminDashboardStats(): Promise<DashboardStats> { const cards = !env.isSupabaseConfigured ? (await readStore()).cards : (await getCards({ perPage: 10000 })).items; const inquiries = !env.isSupabaseConfigured ? (await readStore()).inquiries : (await getAllInquiries({ perPage: 10000 })).items; return { totalCards: cards.length, availableCards: cards.filter((card) => card.availabilityStatus === "AVAILABLE").length, reservedCards: cards.filter((card) => card.availabilityStatus === "RESERVED").length, soldCards: cards.filter((card) => card.availabilityStatus === "SOLD").length, totalInquiries: inquiries.length, newInquiries: inquiries.filter((inquiry) => inquiry.status === "NEW").length, totalSalesMinor: cards.filter((card) => card.availabilityStatus === "SOLD").reduce((sum, card) => sum + card.priceMinor, 0) }; }
export async function getAuditLogs(limit = 100) { if (!env.isSupabaseConfigured) return (await readStore()).auditLogs.sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, limit); const { data, error } = await createAdminClient().from("audit_logs").select().order("created_at", { ascending: false }).limit(limit); if (error) throw error; return (data ?? []).map((entry) => snakeToAuditLog(asRecord(entry))); }
function makeAudit(actorId: string | null, action: string, entityType: string, entityId: string | null, beforeState: Record<string, unknown> | null, afterState: Record<string, unknown> | null): AuditLog { return { id: uuid(), actorId, action, entityType, entityId, beforeState, afterState, createdAt: isoNow() }; }
...
export async function getAppSettings(): Promise<AppSetting[]> { if (!env.isSupabaseConfigured) return (await readStore()).appSettings; const { data, error } = await createAdminClient().from("app_settings").select(); if (error) throw error; return (data ?? []).map((entry) => snakeToAppSetting(asRecord(entry))); }
export async function updateAppSettings(settings: Record<string, unknown>, actorId: string): Promise<AppSetting[]> { await assertAdmin(actorId); const now = isoNow(); const entries = Object.entries(settings).map(([key, value]) => ({ key, value, updatedAt: now, updatedBy: actorId })); if (!env.isSupabaseConfigured) return updateStore((store) => { for (const entry of entries) { const index = store.appSettings.findIndex((current) => current.key === entry.key); if (index >= 0) store.appSettings[index] = entry; else store.appSettings.push(entry); } return entries; }); const { data, error } = await createAdminClient().from("app_settings").upsert(entries.map((entry) => ({ key: entry.key, value: entry.value, updated_at: entry.updatedAt, updated_by: entry.updatedBy }))).select(); if (error) throw error; await appendAuditLog({ actorId, action: "UPDATE_SETTINGS", entityType: "APP_SETTING", entityId: null, beforeState: null, afterState: settings }); return (data ?? []).map((entry) => snakeToAppSetting(asRecord(entry))); }
